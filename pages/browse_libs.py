...
from logic.functions import save_session_state, load_session_state, get_email_user_name
...
for row in query:
    ...
    row[ColName.inspect_link] = f"http://{DOMAIN}/{page_name}?{LIB_ID_URL_KEY}={row[LIB_ID_URL_KEY]}"  # noqa
...
col_order = list(column_config)  # Same order as in the column config dictionary.

df = pd.DataFrame(rows, columns=col_order)

possible_filters = [ColName.experimenter] # TODO ColName.characs
dynamic_filters = DynamicFilters(df, filters=possible_filters, filters_name=Ck.LIB_FILTERS)
