...
from logic.form_fields.new_lib import LibNameField, FilmPhysicalNameField, \
    ...
    ZipUploadField, SputteringSystemField, TargetField, FilmLayerFunctionField,\
    StoichiometryField, \
...
zip_data_list: list[LayerZipData] = sorted(fake_data,
                                           key=lambda x: x.position_from_buffer)
assert [d.position_from_buffer for d in zip_data_list] == list(
    range(len(zip_data_list)))

st.divider()


@dataclass
class AdditionalLayerData(DialogData):
    stoichio: str
    ...
    def form_to_session(cls):
        ...
        if st.button('OK', disabled=not all(f.is_valid for f in fields_)):
            ...
            data_ = cls(fields_, stoichio_fld.value, function_fld.value,
                        sputtering_data,
                        position)
...
if base_fields_valid:
    ...
    for zip_data in zip_data_list:
        ...
        try:
            additional_data = next(
                d for d in additional_list
            ...
            if flex.button("Fill Layer Info",
                           key=zip_data.position_from_buffer):
                current_layer_num = zip_data.position_from_buffer
                sess[Sk.CURRENTLY_FILLED_LAYER_POSITION] = current_layer_num
...
if st.button("Submit Library", disabled=not can_submit):
    additional_list = sorted(additional_list,
                             key=lambda d: d.position_from_buffer)
    ...
    film = Film.new(film_name_fld.value, made_on_fld.value, made_by_fld.value,
                    substrate,
                    library)
    user_uploaded_file = UserUploadedFile.from_streamlit_uploaded_file(
        zip_upload_fld.value)
    film_layers = [
        FilmLayer.new(
