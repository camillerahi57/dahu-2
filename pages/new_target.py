...
from logic.form_fields.new_target import MadeAtField, ExperimenterEmailField, TargetNameField, CommentField, \
    StoichiometryField, DiscCenterX, DiscCenterY, RadiusField, RectangleFirstVertexX, RectangleFirstVertexY, \
    RectangleOppositeVertexX, RectangleOppositeVertexY, PhotoUploadField, TargetDiameterMillimeters, PolygonDataText
from logic.functions import save_session_state, load_session_state, disc_patch_to_scatter, \
...
with col1:
    ...
    for patch_number, form in enumerate(sess[Sk.PATCH_FORMS]):
        ...
        with col11:
            ...
            if form.__class__.__name__ == PolygonForm.__name__:
                vertices_text = ' '.join([f'({x}, {y})' for x, y in form.vertices()])
    ...
    if len(sess[Sk.PATCH_FORMS]) == 0:
        st.markdown("- Start with the patches at the back.\n"
                    ...
                    "- Open the target photo with Microsoft Paint to get pixel positions "
                    "(on the bottom left).")
...
with col2:
    ...
    # if photo_upload_fld.value:
    #     # Not working (not the right positions and orientation (André's image received on 2026-03-23)).
...
st.divider()
fields = [made_at_fld, email_fld, target_name_fld, comment_fld, photo_upload_fld, target_diameter]
...
if st.button("Submit", disabled=not can_submit, type="primary"):
    sess[Ck.LAST_EMAIL_USED] = email_fld.value  # Save last used email for future autofill.
    target = Target.new(
    ...
    with db.atomic():  # Save everything in a single transaction (everything or nothing).
        target.save()
        for patch_number, form in enumerate(sess[Sk.PATCH_FORMS]):
            ...
            if form.__class__.__name__ == DiscForm.__name__:
                ...
                patch, disc = Patch.new_disc_patch(
                    form.stoichio, x_y_radius, target, patch_number, is_target_base
                )
                # Save the patch (order matters):
                patch.save()
                disc.save()
            # If polygon:
            if form.__class__.__name__ in [AlignedRectangleForm.__name__, PolygonForm.__name__]:
                vertices = form.vertices()
                patch, polygon, vertices = Patch.new_polygon_patch(
                    form.stoichio, vertices, patch_number, target, is_target_base
                )
                # Save the patch (order matters):
                patch.save()
                polygon.save()
